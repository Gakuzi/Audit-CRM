...
const ContactCard: React.FC<{ contact: ContactPerson; onShare: () => void; onSelect: () => void; }> = ({ contact, onShare, onSelect }) => {
    ...
    return (
        <div onClick={onSelect} className="p-3 border rounded-md flex gap-4 transition-colors bg-slate-50 hover:bg-blue-50 cursor-pointer">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-sm text-slate-600 truncate">{contact.role}</p>
                    </div>
...
const CompanyProfileModal: React.FC<CompanyProfileModalProps> = ({ isOpen, onClose, project, isAuditor, onContactSelect }) => {
  ...
  return (
    <>
        <Modal 
            isOpen={isOpen} 
            onClose={onClose} 
            title={isEditing ? 'Редактирование профиля' : (profile.company_name || 'Профиль компании')}
            footer={isAuditor && (
                isEditing ? (
                    <>
                                ...
                                <div className="space-y-3">
                                    {filteredContacts.map(contact => (
                                        <ContactCard 
                                            key={contact.id} 
                                            contact={contact} 
                                            onShare={() => setContactToShare(contact)} 
