    ...
    return (
        <aside className="bg-white rounded-lg shadow-md p-4 sticky top-6 h-[calc(100vh-3rem)] flex flex-col">
                ...
                {loading ? <Spinner /> : (
                    events.length > 0 ? (
                        <div className="divide-y divide-gray-200">
                            {events.map(event => <EventItem key={event.id} event={event} contacts={companyProfile?.contacts || []} onReply={handleReply} onQuoteClick={handleQuoteClick} onContactClick={() => {}} />)}
                        </div>
                    ) : (
            ...
            <div className="pt-2 border-t border-gray-200">
                ...
                {(user || isGuest) && projectId && project && task ? <AddEventForm user={user} providerToken={null} context={{...context, projectId, item: task}} quotedEvent={null} onClearQuote={() => {}} onNewEvent={() => {}} project={project} contacts={companyProfile?.contacts || []} isGuest={isGuest} onAddSubTaskRequest={() => {}} onContactsUpdate={() => {}} /> : <p className="text-sm text-center text-gray-500">Войдите, чтобы оставлять комментарии.</p>}
            </div>
