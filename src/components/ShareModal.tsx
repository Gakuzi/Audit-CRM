...
const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, project }) => {
    ...
    const shareText = `Ссылка для гостевого доступа к аудиту «${project.name}».

ВНИМАНИЕ: Эта ссылка — как ключ от кабинета. Любой, у кого она есть, сможет комментировать, планировать встречи и согласовывать этапы от вашего имени.

Храните ее в безопасности и не передавайте третьим лицам.

${shareUrl}`;

    return (
    <Modal isOpen={isOpen} onClose={onClose} title="Поделиться проектом">
        <p className="text-sm text-gray-600 mb-4">Отправьте эту ссылку заказчику. Он сможет комментировать, планировать встречи и согласовывать этапы, используя только эту ссылку.</p>
