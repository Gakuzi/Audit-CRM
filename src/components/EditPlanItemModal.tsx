...
import { PlanItem, PlanItemType, Profile, ContactPerson } from '../types';
...
const EditPlanItemModal: React.FC<EditPlanItemModalProps> = ({ isOpen, onClose, onUpdateItem, item, date, profile, providerToken, contacts }) => {
  ...
  const renderMeetingForm = () => (
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-4 text-gray-600"><FaMapMarkerAlt size={20} className="flex-shrink-0" /><input type="text" value={editedItem.data?.location || ''} onChange={e => handleDataChange('location', e.target.value)} className="input w-full" placeholder="Место или ссылка"/></div>
          ...
          <div className="flex items-start gap-4 text-gray-600"><FaAlignLeft size={20} className="flex-shrink-0 mt-2" /><textarea value={editedItem.description || ''} onChange={e => setEditedItem({...editedItem, description: e.target.value})} className="input w-full" rows={4} placeholder="Описание или повестка" /></div>
