...
const TaskDetailView: React.FC<TaskDetailViewProps> = ({ isOpen, onClose, user, providerToken, context, onEventCountChange, onUpdateTask, isAuditor, isGuest, project, onSubTaskAdded }) => {
    ...
    const handleNewAiEvent = async (eventPayload: Partial<Event>) => {
        ...
        const { data, error } = await supabase.from('events').insert(fullPayload).select('*, parent:events!parent_event_id(content, author_email)').single();
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`public:events:task_id=eq.${context.item.id}`)
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'events', filter: `task_id=eq.${context.item.id}` }, (payload) => {
                setEvents(prev => {
                    if (prev.find(e => e.id === payload.new.id)) return prev;
                    return [...prev, payload.new as Event];
                });
            })
            .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'events', filter: `task_id=eq.${context.item.id}` }, (payload) => {
                setEvents(prev => prev.filter(e => e.id !== payload.old.id));
            })
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'events', filter: `task_id=eq.${context.item.id}` }, (payload) => {
                ...
                <div className="flex-1 flex flex-col min-w-0" style={{minHeight: 0}}>
                    ...
                    <footer className="flex-shrink-0 p-2 bg-gray-50 border-t lg:border-l">
                        <AddEventForm 
                          ...
                          project={project} 
                          isGuest={isGuest} 
                          onAddSubTaskRequest={() => setIsAddEventModalOpen(true)}
                        />
                    </footer>
                </div>
            </div>
            {(isAuditor || isGuest) && <AddEventModal isOpen={isAddEventModalOpen} onClose={() => setIsAddEventModalOpen(false)} onAddSubTask={handleAddSubTask} />}
