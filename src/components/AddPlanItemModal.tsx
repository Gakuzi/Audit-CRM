...
interface AddPlanItemModalProps {
  ...
  week: Week;
  date: string;
  parentTaskId?: string; // New prop to identify subtask creation
}
...
const AddPlanItemModal: React.FC<AddPlanItemModalProps> = ({ isOpen, onClose, onUpdatePlan, week, date, parentTaskId }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (title.trim() && itemType) {
      ...
      if (parentTaskId) {
        // This is a subtask, find parent and add it
        let parentFound = false;
        for (const planDate in newPlan) {
            const day = newPlan[planDate];
            const parentIndex = day.tasks.findIndex(t => t.id === parentTaskId);
            if (parentIndex > -1) {
                if (!day.tasks[parentIndex].sub_tasks) {
                    day.tasks[parentIndex].sub_tasks = [];
                }
                day.tasks[parentIndex].sub_tasks!.push(newItem);
                parentFound = true;
                break;
            }
        }
        if (!parentFound) {
            alert("Ошибка: родительская задача не найдена.");
            return;
        }
      } else {
        // This is a main task, add it to the date
        if (!newPlan[date]) {
            newPlan[date] = { tasks: [] };
        }
  ...
  const renderForm = () => {
    ...
    const modalTitle = parentTaskId ? 'Добавить подзадачу' : `Добавить: ${currentType?.name}`;
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={parentTaskId ? 'Добавить подзадачу' : 'Добавить событие в план'}>
      {step === 'select' ? (
