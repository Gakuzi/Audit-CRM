...
import { FaChevronDown, FaChevronUp, FaEdit, FaTrash, FaPlus, FaBrain, FaCheckCircle, FaCalendarAlt, FaPaperPlane, FaShareAlt } from 'react-icons/fa';
...
const WeekCard: React.FC<WeekCardProps> = ({ week, isAuditor, isGuest, onUpdatePlan, onTaskSelect, onDeleteRequest, onUpdateRequest, onGenerateReport, onSentForApproval }) => {
  ...
  const handleStatusChange = async (newStatus: Week['status'], bypassConfirmation = false) => {
    ...
    let guestName = '';
    if (isGuest) {
        guestName = localStorage.getItem('guestName') || 'Гость';
    }
    ...
    if (error) {
      ...
      if (isGuest && (newStatus === 'approved' || newStatus === 'rejected')) {
          // Fire off notification without blocking UI
          supabase.functions.invoke('send-guest-status-change-notification', {
            body: {
  ...
  const canModifyStatus = isAuditor || (isGuest && (week.status === 'pending_approval' || week.status === 'rejected'));
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300">
           ...
           <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={statusRef}>
                 <button 
                    onClick={(e) => {
                        e.stopPropagation();
                        if (canModifyStatus) setIsStatusDropdownOpen(prev => !prev);
                    }}
                    className={`px-3 py-1 font-semibold rounded-full ${statusConfig[week.status].color} ${canModifyStatus ? 'cursor-pointer hover:ring-2 ring-offset-1' : ''}`}
                    title={canModifyStatus ? "Изменить статус" : ""}
                 >
                    {statusConfig[week.status].label}
                </button>
                {canModifyStatus && isStatusDropdownOpen && (
                    <div className="absolute top-full right-0 mt-2 w-48 bg-white border rounded-md shadow-lg z-20">
                        {isAuditor ? (
                            ...
                            <>
                                <a onClick={(e) => {e.stopPropagation(); handleStatusChange('approved')}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer">{statusConfig.approved.label}</a>
                                <a onClick={(e) => {e.stopPropagation(); handleStatusChange('rejected')}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer">{statusConfig.rejected.label}</a>
                            </>
                        )}
                    </div>
                )}
      ...
      {isExpanded && (
        <div className="p-4 bg-gray-50/50">
            {week.rejection_comment && (week.status === 'rejected' || (week.status === 'draft' && isAuditor)) && (
            ...
            <div className="mt-6 pt-4 border-t flex justify-between items-center flex-wrap gap-2">
                <div className="flex items-center gap-2">
                 ...
                 {isAuditor && (
                    <div className="flex items-center gap-2">
                        {week.status === 'pending_approval' && <button onClick={() => onSentForApproval(week)} className="btn-secondary flex items-center gap-2 text-sm"><FaShareAlt/> Поделиться</button>}
                        {(week.status === 'draft' || week.status === 'approved') && (
