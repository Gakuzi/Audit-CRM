...
const WeekCard: React.FC<WeekCardProps> = ({ week, isAuditor, isGuest, project, profile, companyProfile, providerToken, onUpdatePlan, onTaskSelect, onDeleteRequest, onUpdateRequest, onGenerateReport, onSentForApproval, onUpdateTask, onContactClick, onContactsUpdate }) => {
  const isCurrentWeek = () => {
  ...
  const handleStatusChange = async (newStatus: Week['status'], bypassConfirmation = false) => {
    ...
    if (error) {
      ...
      const authorName = isAuditor ? (profile?.full_name || 'Аудитор') : (localStorage.getItem('guestName') || 'Гость');
      sendGuestStatusChangeNotification(project, { ...week, ...updateData }, newStatus, authorName, reason, window.location.origin);
      onUpdateRequest();
    }
             ...
             <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={statusRef}>
                   <button 
                      ...
                      className={`px-3 py-1 font-semibold rounded-full text-xs ${statusConfig[week.status].color} ${isAuditor ? 'cursor-pointer hover:ring-2 ring-offset-1' : ''}`}
        ...
        {isExpanded && (
          <div className="p-4 bg-slate-50/50">
              ...
              {week.rejection_comment && week.status === 'rejected' && (
                  <div className="my-4 p-3 bg-red-50 border-l-4 border-red-500 text-red-800">
